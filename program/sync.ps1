$repoPath = "."
Write-Host "`n Starte Git-Sync fuer $repoPath"
...
if ($branches.Count -eq 1) {
    ...
    # Auswahl treffen
    $selection = Read-Host "`nBitte waehle eine Branch-Nummer"
    if ($selection -match '^\d+$' -and [int]$selection -le $branches.Count -and [int]$selection -gt 0) {
        $branch = $branches[$selection - 1]
        Write-Host "`n Gewaehlter Branch: $branch"
    }
    else {
        Write-Host "`nUngueltige Auswahl, Skript wird beendet."
        exit
    }
}
...
Write-Host "Lokale Aenderungen werden gesichert..."
...
if ($pullOutput -match "CONFLICT") {
    Write-Host "`nMerge-Konflikt erkannt! Oeffne VS Code..." 
    code .  # VS Code öffnen
    git status
    exit
}
...
Write-Host "Stash zurueckholen..."
git stash pop

# Prüfe auf neue Änderungen, die committet werden müssen
$status = git status --porcelain
if ($status) {
    Write-Host "Neue Aenderungen erkannt, committe und pushe..."
    git add .
    git commit -m "Auto-Sync Update"
    git push origin $branch
}
else {
    Write-Host "Keine neuen Aenderungen, Repository ist aktuell!"
}
