...
if ($branches.Count -eq 0) {
    Write-Host "❌ Keine Branches gefunden! Skript wird beendet." -ForegroundColor Red
    exit
...
if (-not $branch) {
    Write-Host "`n❌ Fehler: Kein Branch ausgewählt! Skript wird beendet." -ForegroundColor Red
    exit
}

# Wechsle zum ausgewählten Branch
Write-Host "🔄 Wechsel zu Branch: $branch..."
git checkout $branch
